...
_parser.add_argument(
    ...
    help="Append 7 per-structure aleatoric uncertainty features to the classifier input. "
         "Not necessary for the baseline (radiomic + geometric features only)."
)
_parser.add_argument(
    "--uncertainty-csv",
    default="output/asoca/uncertainty_analysis_aleatoric/metrics/per_structure_uncertainty.csv",
    help="Path to per_structure_uncertainty.csv produced by compute_uncertainty_aleatoric.py "
         "(must contain mean_aleatoric column)."
)
_parser.add_argument(
...
print(f"  Shape    : {_unc_df.shape}   "
      f"(will be {len(subjects) * 7} rows × 5 cols once ASOCA uncertainty is complete)")
print(_unc_df.head(14).to_string(index=False))

# Single pivot: one aleatoric value per (sample_id, class_id)
_unc_pivot = _unc_df.pivot(index="sample_id", columns="class_id", values="mean_aleatoric")
_unc_pivot.columns.name = None

print(f"Uncertainty pivot table verification")
print(f"  Pivot shape : {_unc_pivot.shape}   (rows = sample_id, cols = class_id 1–7)")
print(f"  Column IDs  : {list(_unc_pivot.columns)}")
print(f"  Sample 0    :\n{_unc_pivot.iloc[0].to_string()}")
...
if missing_ids:
    print(f"\n  [WARN] {len(missing_ids)} subjects have no uncertainty entry yet "
          ...
          f"Their unc_aleatoric_* values will be NaN until the ASOCA uncertainty run is complete.")
else:
...
for subj in subjects:
    ...
    # One aleatoric uncertainty feature per structure: H(mean_p) − JSD
    sid = subj["sample_idx"]
    for L, name in class_mapping.items():
        row[f"unc_{name}"] = (
...
# debug: confirm aleatoric uncertainty columns are present
...
_n_unc_feats = _n_structs                                  # 7 (aleatoric per structure)
...
print(f"  Uncertainty feats : {_n_unc_feats}  (aleatoric per structure × {_n_structs} structures)")
...
def objective(trial):
    ...
    # Append aleatoric uncertainty per structure if the flag is on (7 features total)
...
_exp_tag = "uncertainty_aleatoric" if USE_UNCERTAINTY_FEATURES else "baseline"
_result_rows = []
...
# 7 per-structure aleatoric features (H(mean_p) − JSD) appended to the baseline feature vector.
# This file consumes the "per_structure_uncertainty" CSV produced by compute_uncertainty_aleatoric.py
