"""
Post-processing script: compute a single global entropy uncertainty value per image.

Variant of compute_uncertainty.py that produces ONE scalar per subject instead of
7 per-structure values.  The scalar is the mean multiclass predictive entropy over all
foreground voxels (consensus argmax > 0), with no per-class masking or threshold sweep.

Output CSV: metrics/per_case_uncertainty.csv with columns [sample_id, mean_entropy].
Consumed by Geo-Radio-Classification_entropy_only_one.py which adds a single
"unc_entropy" feature to the classifier rather than 7 per-structure columns.
"""
...
# Global foreground entropy (one value per image)

def compute_global_foreground_entropy(
    entropy: np.ndarray,
    consensus: np.ndarray,
    sample_id: int,
) -> dict:
    """Mean multiclass predictive entropy over all foreground voxels (consensus > 0).

    No per-class masking or threshold sweep — just one scalar per image.

    Args:
        entropy:   (D, H, W)  multiclass predictive entropy of mean probability map
    ...
    Returns a dict with keys: sample_id, mean_entropy, voxel_count
    """
    fg_mask = consensus > 0
    voxel_count = int(fg_mask.sum())
    mean_entropy = float(entropy[fg_mask].mean()) if voxel_count > 0 else float("nan")
    print(f"  Global foreground entropy: mean_entropy = {mean_entropy:.6f}  fg voxels = {voxel_count}")
    return {"sample_id": sample_id, "mean_entropy": mean_entropy, "voxel_count": voxel_count}
...
def process_sample(
    ...
    Returns a dictionary with:
        "entropy_row": dict {sample_id, mean_entropy, voxel_count} — global foreground entropy
        "per_case":    dict with global per-case multiclass entropy stats (diagnostic)
    """
    print(f"\n--- Sample {sample_id} ---")
    ...
    # Single global foreground entropy scalar (no threshold sweep, no per-class masking)
    entropy_row = compute_global_foreground_entropy(entropy, consensus, sample_id)

    # Diagnostic: also store whole-volume entropy alongside foreground
    mean_entropy_all = float(entropy.mean())
    print(f"  Entropy — all voxels: {mean_entropy_all:.4f}  fg only: {entropy_row['mean_entropy']:.4f}")

    per_case_row = {
        "sample_id":        sample_id,
        "mean_entropy_all": mean_entropy_all,
        "mean_entropy_fg":  entropy_row["mean_entropy"],
    }
    ...
    return {"entropy_row": entropy_row, "per_case": per_case_row}
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute a single global entropy uncertainty value per image across multiple Anatomix seeds."
    )
    parser.add_argument(
        "--uncertainty_root",
    ...
    parser.add_argument(
        "--out_dir",
        default="output/asoca/uncertainty_analysis_entropy_only_one",
    ...
    for sample_id in sample_ids:
        ...
        all_entropy_rows.append(result["entropy_row"])
        all_per_case.append(result["per_case"])

    # Write output CSVs
    entropy_csv = out_dirs["metrics"] / "per_case_uncertainty.csv"
    diag_csv    = out_dirs["metrics"] / "per_case_entropy_diagnostic.csv"

    # One row per subject: sample_id, mean_entropy (global foreground), voxel_count
    write_csv(
        all_entropy_rows,
        ["sample_id", "mean_entropy", "voxel_count"],
        entropy_csv,
    )

    # Diagnostic CSV (all-voxel vs foreground breakdown)
    write_csv(
        all_per_case,
    ...
    print(f"\nMetrics saved:")
    print(f"  {entropy_csv}  (one global entropy value per subject)")
    print(f"  {diag_csv}  (diagnostic: all-voxel vs foreground)")
    print("\nDone.")
