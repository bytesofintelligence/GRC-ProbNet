...
def train(args):
    ...
    dataset_train_base = ImageSegmentationOneHotDataset(args.train,
                                                        ...
                                                        model_checkpoint=args.anatomix_model)
...
def test_uncertainty(args):
    ...
    # Load the trained STN once - which stays frozen for all seeds
    config.stn.load_state_dict(torch.load(args.model + '/stn.pt'))
    config.stn.eval()
    ...
    for ckpt_path in args.anatomix_checkpoints:
        ...
        # build the dataset for this seed's Anatomix checkpoint.
        # fixed_crop_size=saved_crop_size ensures the crop size matches the trained STN
        # Crop centre (centroid) will differ per seed based on the Anatomix segmentation
...
if __name__ == '__main__':
    ...
    # Logging args
    parser.add_argument('--out', default='output/mm-whs/full-stn', help='output root directory')
    parser.add_argument('--model', default='output/mm-whs/full-stn/train/model', help='model directory')
    ...
    parser.add_argument('--anatomix-model', default=None,
                        help='Explicit Anatomix checkpoint to use for segmentation inference '
                             '(overrides automatic best-checkpoint selection)')
