...
_parser.add_argument(
    ...
    help="Append 1 global JSD uncertainty feature per image to the classifier input. "
         "Not necessary for the baseline (radiomic + geometric features only)."
)
_parser.add_argument(
    "--uncertainty-csv",
    default="output/asoca/uncertainty_analysis_kl_only_one/metrics/per_case_uncertainty.csv",
    help="Path to per_case_uncertainty.csv produced by compute_uncertainty_kl_only_one.py."
)
_parser.add_argument(
...
# Load global-JSD CSV (one row per subject: sample_id, mean_kl, voxel_count)
# Always loaded so df_full is consistent; USE_UNCERTAINTY_FEATURES controls
# whether "unc_kl" enters selected_cols in objective().
...
# Simple Series: sample_id → mean_kl (no pivot needed — already one row per subject)
_unc_series = _unc_df.set_index("sample_id")["mean_kl"]
...
if missing_ids:
    print(f"\n  [WARN] {len(missing_ids)} subjects have no uncertainty entry yet "
          ...
          f"Their unc_kl value will be NaN until the ASOCA uncertainty run is complete.")
else:
...
for subj in subjects:
    ...
    # Single global JSD feature — one scalar per image, no per-structure breakdown
    sid = subj["sample_idx"]
    row["unc_kl"] = (
...
print(f"  ['unc_kl']")
print(f"  Example subject 0: unc_kl = {df_full['unc_kl'].iloc[0]:.6f}")
...
_n_unc_feats = 1                                           # 1 global JSD scalar
...
print(f"  Uncertainty feats : {_n_unc_feats}  (one global foreground JSD per image)")
...
_exp_tag = "uncertainty_kl_one" if USE_UNCERTAINTY_FEATURES else "baseline"
_result_rows = []
