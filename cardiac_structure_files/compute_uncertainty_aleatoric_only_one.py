"""
Post-processing script: compute a single global aleatoric uncertainty value per image.

Variant of compute_uncertainty_aleatoric.py that produces ONE scalar per subject instead of
7 per-structure values.  The scalar is the mean aleatoric uncertainty (H(mean_p) − JSD)
over all foreground voxels (consensus argmax > 0), with no per-class masking or threshold sweep.

Aleatoric uncertainty = H(mean_p) − avg_kl (clipped to 0 for numerical safety).
Theoretically non-negative; clip handles small numerical errors from avg_kl ≈ JSD.

Output CSV: metrics/per_case_uncertainty.csv with columns [sample_id, mean_aleatoric, voxel_count].
Consumed by Geo-Radio-Classification_aleatoric_only_one.py which adds a single "unc_aleatoric"
...
# Global foreground aleatoric (one value per image)

def compute_global_foreground_aleatoric(
    aleatoric_map: np.ndarray,
    consensus: np.ndarray,
    sample_id: int,
) -> dict:
    """Mean aleatoric uncertainty over all foreground voxels (consensus > 0).

    No per-class masking or threshold sweep — just one scalar per image.
    Aleatoric = H(mean_p) − avg_kl (clipped to 0), already computed by caller.

    Args:
        aleatoric_map: (D, H, W)  voxel-wise aleatoric uncertainty (H − JSD, clipped to 0)
    ...
    Returns a dict with keys: sample_id, mean_aleatoric, voxel_count
    """
    fg_mask = consensus > 0
    voxel_count = int(fg_mask.sum())
    mean_aleatoric = float(aleatoric_map[fg_mask].mean()) if voxel_count > 0 else float("nan")
    print(f"  Global foreground aleatoric: mean_aleatoric = {mean_aleatoric:.6f}  fg voxels = {voxel_count}")
    return {"sample_id": sample_id, "mean_aleatoric": mean_aleatoric, "voxel_count": voxel_count}
...
def process_sample(
    ...
    Returns a dictionary with:
        "aleatoric_row": dict {sample_id, mean_aleatoric, voxel_count} — global foreground aleatoric
    ...
    # Single global foreground aleatoric scalar (no threshold sweep, no per-class masking)
    aleatoric_row = compute_global_foreground_aleatoric(aleatoric_map, consensus, sample_id)
    ...
    return {"aleatoric_row": aleatoric_row, "per_case": per_case_row}
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute a single global aleatoric uncertainty value per image across multiple Anatomix seeds."
    )
    parser.add_argument(
        "--uncertainty_root",
    ...
    parser.add_argument(
        "--out_dir",
        default="output/asoca/uncertainty_analysis_aleatoric_only_one",
    ...
    for sample_id in sample_ids:
        ...
        all_aleatoric_rows.append(result["aleatoric_row"])
        all_per_case.append(result["per_case"])

    # Write output CSVs
    aleatoric_csv = out_dirs["metrics"] / "per_case_uncertainty.csv"
    case_csv      = out_dirs["metrics"] / "per_case_entropy.csv"

    # One row per subject: sample_id, mean_aleatoric (global foreground aleatoric), voxel_count
    write_csv(
        all_aleatoric_rows,
        ["sample_id", "mean_aleatoric", "voxel_count"],
        aleatoric_csv,
    )
    ...
    print(f"\nMetrics saved:")
    print(f"  {aleatoric_csv}  (one global aleatoric value per subject)")
