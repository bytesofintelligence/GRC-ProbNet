...
dimension. Per-structure uncertainty is computed with TWO metrics per structure:

  mean_entropy — masked mean binary class entropy on the mean probability map.
                 Captures total uncertainty (aleatoric + epistemic).
  mean_kl      — masked mean JSD (average KL divergence from mean across seeds).
                 Captures epistemic uncertainty (seed disagreement).

Aleatoric uncertainty = H(mean_p) − JSD  (clipped to 0 for numerical safety).
Per structure c, the aleatoric map is masked to voxels where mean_probs[...,c] > τ and
averaged.  Threshold τ is swept to maximise CV of aleatoric values across samples.
This produces 7 uncertainty features per subject (one per structure).
"""
...
def compute_kl_divergence_per_class(p: np.ndarray, q: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    """Per-class KL divergence contribution without summing over classes.
...
# Threshold sweep — both binary class entropy and JSD per structure
...
def compute_threshold_sweep(
    ...
    """Masked mean aleatoric uncertainty for each foreground class at every threshold τ.

    aleatoric = clip(H(mean_p) − avg_kl, 0) — voxel-wise, shape (D,H,W).
    For class c and threshold τ, restrict to voxels where mean_probs[..., c] > τ.
    Background (class 0) is skipped.

    Args:
        mean_probs:  (D, H, W, C) mean probability map across seeds
        aleatoric:   (D, H, W)   voxel-wise aleatoric uncertainty map
        sample_id:   integer sample index for labelling rows
        thresholds:  list of float probability thresholds

    Returns a list of dicts with keys:
            sample_id, class_id, class_name, threshold, mean_aleatoric, voxel_count
    """
    rows = []
    ...
    for class_id in range(1, C):  # skip background
        ...
        for tau in thresholds:
            ...
            mean_aleatoric = float(aleatoric[mask].mean()) if voxel_count > 0 else float("nan")
            rows.append({
                "sample_id":      sample_id,
                "class_id":       class_id,
...
def select_threshold(
    ...
    """Pick the threshold with the highest CV of per-structure aleatoric values.

    Thresholds where any (sample, class) pair has fewer than min_voxel_count voxels
    are rejected. Falls back to the smallest threshold if all candidates are rejected.
    """
    ...
    for tau in thresholds:
        ...
        if any(r["voxel_count"] < min_voxel_count for r in rows):
            continue
        vals = [r["mean_aleatoric"] for r in rows if not np.isnan(r["mean_aleatoric"])]
        if len(vals) < 2:
            continue
        arr = np.array(vals)
        mean_a = arr.mean()
...
def print_threshold_summary(
    ...
    header = f"  {'τ':>7}  {'Mean Aleatoric':>14}  {'Std':>9}  {'CV':>8}  {'Min voxels':>11}"
    print(header)
    print("  " + "-" * (len(header) - 2))
    ...
    for tau in thresholds:
        ...
        if vals:
            ...
            mean_a = arr.mean()
            std_a  = arr.std()
            cv = std_a / mean_a if mean_a > 0 else float("nan")
        else:
            mean_a = std_a = cv = float("nan")

        min_v = min(voxel_counts) if voxel_counts else 0
        marker = "  <-- recommended" if tau == selected_tau else ""
        print(f"  {tau:>7.3f}  {mean_a:>14.6f}  {std_a:>9.6f}  {cv:>8.4f}  {min_v:>11d}{marker}")

    print(f"\nRecommended threshold: τ = {selected_tau}")
    print(
        "Rationale: maximises CV of per-structure aleatoric uncertainty\n"
...
def process_sample(
    ...
    # Compute voxel-wise aleatoric map: H(mean_p) − JSD, clipped to 0
    # Theoretically non-negative, clip handles small numerical errors from avg_kl ≠ exact JSD
    aleatoric_map = np.maximum(entropy - avg_kl, 0.0)  # (D, H, W)
    print(f"  Aleatoric map: mean = {aleatoric_map.mean():.6f}  max = {aleatoric_map.max():.6f}")
    save_nifti(
        aleatoric_map,
        ref_img,
        out_dirs["kl_divergence"] / f"{sid}_aleatoric_map.nii.gz",
    )

    # Per-class aleatoric maps: clip(H_c(mean_p_c) − avg_per_class_kl_c, 0) for each class
    avg_per_class_kl = np.stack(
        [compute_kl_divergence_per_class(soft, mean_probs) for soft in soft_maps],
        axis=0,
    ).mean(axis=0)  # (D, H, W, C)
    C = mean_probs.shape[-1]
    for class_id in range(C):
        h_c = compute_binary_class_entropy(mean_probs[..., class_id])  # (D, H, W)
        aleatoric_c = np.maximum(h_c - avg_per_class_kl[..., class_id], 0.0)
        save_nifti(
            aleatoric_c,
            ref_img,
            out_dirs["per_class_aleatoric"] / f"{sid}_class_{class_id}_aleatoric.nii.gz",
        )
    print(f"  Saved: per-class aleatoric maps for {C} classes")

    # Per-structure threshold sweep on the aleatoric map
    sweep_rows = compute_threshold_sweep(mean_probs, aleatoric_map, sample_id, thresholds)
    print(
        f"  Threshold sweep: {len(thresholds)} thresholds × {C - 1} foreground "
        f"classes = {len(sweep_rows)} rows"
    )

    # Diagnostic global entropy
...
def main():
    parser = argparse.ArgumentParser(
        description="Compute per-structure aleatoric uncertainty (H − JSD) from multiple Anatomix seeds."
    )
    parser.add_argument(
        "--uncertainty_root",
    ...
    parser.add_argument(
        "--out_dir",
        default="output/asoca/uncertainty_analysis_aleatoric",
    ...
    out_dirs = {
        ...
        "kl_divergence":        out_root / "kl_and_aleatoric_divergence_maps",
        ...
        "per_class_aleatoric":  out_root / "per_class_aleatoric_maps",
    ...
    # Final per-structure aleatoric at the automatically selected threshold (7 values per subject)
