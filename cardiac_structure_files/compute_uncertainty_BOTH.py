...
Both share the same soft-mask threshold τ, selected to jointly maximise the
coefficient of variation of both metrics across all (sample, structure) pairs.
This produces 14 uncertainty features per subject (2 × 7 structures).
"""
...
def compute_threshold_sweep(
    ...
    """Masked mean entropy AND mean JSD for each foreground class at every threshold τ.

    For class c and threshold τ, voxels where mean_probs[..., c] > τ form the mask.
    Both metrics are averaged over the same mask:
      mean_entropy — binary class entropy H(p_c) = -p*log(p) - (1-p)*log(1-p)
                     on the mean probability map. Captures total uncertainty.
      mean_kl      — average KL from mean (JSD map) masked to this structure's region.
                     Captures epistemic uncertainty (seed disagreement).
    ...
    Args:
        ...
        avg_kl:      (D, H, W)   JSD map: mean over seeds of KL(seed_i || mean_probs)
        sample_id:   integer sample index for labelling rows
        thresholds:  list of float probability thresholds

    Returns a list of dicts with keys:
            sample_id, class_id, class_name, threshold, mean_entropy, mean_kl, voxel_count
    """
    rows = []
    C = mean_probs.shape[-1]
    for class_id in range(1, C):  # skip background
        p = mean_probs[..., class_id]          # (D, H, W)
        h = compute_binary_class_entropy(p)    # (D, H, W) — total uncertainty per voxel
        for tau in thresholds:
            mask = p > tau
            voxel_count = int(mask.sum())
            mean_ent = float(h[mask].mean())      if voxel_count > 0 else float("nan")
...
def select_threshold(
    ...
    """Pick the threshold that jointly maximises CV of both entropy and JSD across all samples.

    Uses CV_entropy + CV_kl as the selection criterion so neither metric dominates.
    Thresholds where any (sample, class) pair has fewer than min_voxel_count voxels are
    rejected. Falls back to the smallest threshold if all candidates are rejected.
    ...
    for tau in thresholds:
        ...
        ent_vals = [r["mean_entropy"] for r in rows if not np.isnan(r["mean_entropy"])]
        kl_vals  = [r["mean_kl"]      for r in rows if not np.isnan(r["mean_kl"])]
        if len(ent_vals) < 2 or len(kl_vals) < 2:
            continue

        arr_ent = np.array(ent_vals)
        arr_kl  = np.array(kl_vals)
        mean_ent = arr_ent.mean()
        mean_kl  = arr_kl.mean()
        if mean_ent == 0.0 or mean_kl == 0.0:
            continue

        cv_ent = arr_ent.std() / mean_ent
        cv_kl  = arr_kl.std()  / mean_kl
        combined_cv = cv_ent + cv_kl

        if combined_cv > best_combined_cv:
            best_combined_cv = combined_cv
            best_tau = tau
...
def print_threshold_summary(
    ...
    """Print a per-threshold summary table for both entropy and JSD, aggregated over all samples."""

    print("\n--- Threshold Sweep Summary (aggregated over all samples) ---")
    header = (f"  {'τ':>7}  {'Mean H':>9}  {'CV(H)':>7}  "
              f"{'Mean JSD':>10}  {'CV(JSD)':>8}  {'CV sum':>7}  {'Min vox':>8}")
    print(header)
    print("  " + "-" * (len(header) - 2))
    ...
    for tau in thresholds:
        ...
        if ent_vals and kl_vals:
            arr_ent = np.array(ent_vals)
            arr_kl  = np.array(kl_vals)
            mean_ent = arr_ent.mean()
            mean_kl  = arr_kl.mean()
            cv_ent = arr_ent.std() / mean_ent if mean_ent > 0 else float("nan")
            cv_kl  = arr_kl.std()  / mean_kl  if mean_kl  > 0 else float("nan")
            cv_sum = (cv_ent + cv_kl) if not (np.isnan(cv_ent) or np.isnan(cv_kl)) else float("nan")
        else:
            mean_ent = mean_kl = cv_ent = cv_kl = cv_sum = float("nan")

        min_v = min(voxel_counts) if voxel_counts else 0
        marker = "  <-- recommended" if tau == selected_tau else ""
        print(f"  {tau:>7.3f}  {mean_ent:>9.4f}  {cv_ent:>7.4f}  "
              f"{mean_kl:>10.6f}  {cv_kl:>8.4f}  {cv_sum:>7.4f}  {min_v:>8d}{marker}")

    print(f"\nRecommended threshold: τ = {selected_tau}")
    print(
        "Rationale: maximises CV(entropy) + CV(JSD) jointly\n"
...
def main():
    ...
    # Each row has both mean_entropy and mean_kl — 2 features per structure, 14 total per subject
