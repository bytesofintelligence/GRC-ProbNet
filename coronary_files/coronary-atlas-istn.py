...
def _find_best_asoca_fold_checkpoint(fold, base_dir='saved_models/segmentation_asoca'):
    """Return the lowest val loss finetuned Anatomix checkpoint for the given ASOCA fold.

    Checkpoint filenames follow the pattern: finetuned_asoca_<fold>_<loss>.pth
    Falls back to anatomix_trained_asoca_<fold>.pth if no finetuned checkpoints exist.
    """
    fold_dir = os.path.join(base_dir, fold)
    prefix = f'finetuned_asoca_{fold}_'
    candidates = [f for f in os.listdir(fold_dir) if f.startswith(prefix) and f.endswith('.pth')]
    if not candidates:
        fallback = os.path.join(fold_dir, f'anatomix_trained_asoca_{fold}.pth')
        print(f'[Anatomix] No finetuned checkpoints found for {fold}, using {fallback}')
        return fallback
    ...
    print(f'[Anatomix] Auto-selected checkpoint for {fold}: {ckpt}')
    return ckpt
...
def set_up_model_and_preprocessing(phase, args, size_override=None):
    print(separator)
    ...
    # Keep the original config size as the Anatomix sliding-window roi_size so it is
    # independent of the STN input size (which is the computed crop size).
    config['anatomix_roi_size'] = list(config['size'])
    if size_override is not None:
        config['size'] = list(size_override)
...
def process_batch(config, batch_samples, atlas_img, atlas_lab, omega, calculate_metrics=True):
    ...
    loss_train = omega * (loss_seg2atl + loss_atl2seg + reg_weight * reg_term)
...
def process_batch_test(config, config_stn, batch_samples, atlas_img, atlas_lab):
    ...
    atlas_image = atlas_image.repeat(tuple(repeats.astype(int)))
    atlas_labelmap = atlas_labelmap.repeat(tuple(repeats.astype(int)))
    image_prime = batch_samples['labelmap'].to(config.device) 
...
def train(args):
    ...
    print(config)
    # reminder that skip_lcc=True for the coronary setting 
    dataset_train_base = ImageSegmentationOneHotDataset(args.train,
                                                        config.config['num_classes'], config.config['anatomix_roi_size'], config.config['spacing'],
                                                        normalizer=config.normalizer, binarize=config.config['binarize'], augmentation=True,
                                                        model_checkpoint=args.anatomix_checkpoint, skip_lcc=True)

    # Save the crop size computed from the training set so test() can load it.
    train_crop_size = dataset_train_base.crop_size
    os.makedirs(args.model, exist_ok=True)
    crop_size_path = os.path.join(args.model, 'crop_size.json')
    with open(crop_size_path, 'w') as f:
        json.dump({'crop_size': list(train_crop_size)}, f)
    print(f'[TRAIN] crop_size.json saved to {crop_size_path}: crop_size={list(train_crop_size)}')

    # If the computed crop size differs from config['size'], we reinitialise the STN
    # (and its optimiser & scheduler) so the model input matches the actual data shape
    if list(train_crop_size) != config.config['size']:
        print(f'[TRAIN] Crop size {list(train_crop_size)} differs from config size {config.config["size"]} — reinitialising STN')
        stn_input_channels = 2 * (config.config['num_classes'] - 1)
        config.stn = type(config.stn)(
            input_size=list(train_crop_size),
            input_channels=stn_input_channels,
            device=config.device,
        ).to(config.device)
        config.config['size'] = list(train_crop_size)
        parameters = list(config.itn.parameters()) + list(config.stn.parameters())
        config.optimizer = torch.optim.Adam(parameters, lr=config.config['learning_rate'])
        gamma = 0.5 ** (1 / config.config['epoch_decay_steps'])
        config.scheduler = torch.optim.lr_scheduler.ExponentialLR(config.optimizer, gamma)
        print(f'[TRAIN] STN reinitialised with input_size={config.config["size"]}')
    else:
        print(f'[TRAIN] Computed crop_size matches config size {config.config["size"]} — no STN reinit needed')
    ...
    if args.val is not None:
        ...
        dataset_val = ImageSegmentationOneHotDataset(args.val,
                                                     config.config['num_classes'], config.config['anatomix_roi_size'], config.config['spacing'],
                                                     normalizer=config.normalizer, binarize=config.config['binarize'], augmentation=False,
                                                     fixed_crop_size=train_crop_size,
                                                     model_checkpoint=args.anatomix_checkpoint, skip_lcc=True)
...
def test(args):
    ...
    if os.path.exists(crop_size_path):
        with open(crop_size_path) as f:
            saved_crop_size = json.load(f)['crop_size']
        print(f'[TEST] Loaded crop_size={saved_crop_size} from {crop_size_path}')
    else:
        saved_crop_size = None
        print(f'[TEST] crop_size.json not found at {crop_size_path} — falling back to config size')

    # size_override updates config['size'] to the true STN input size before the
    # model is constructed, preventing any shape mismatch when loading weights.
    config = set_up_model_and_preprocessing('TESTING', args, size_override=saved_crop_size)
    print(f'[TEST] STN input_size = {config.config["size"]}, Anatomix roi_size = {config.config["anatomix_roi_size"]}')
    ...
    if args.refine == True:
        # set size_override to ensure the refined STN gets the same input dimensions 
        # as the original STN, preventing any shape mismatch when loading weights
        refine_config = set_up_model_and_preprocessing('REFINEMENT', args, size_override=saved_crop_size)
        config.itn.eval()
...
def test_uncertainty(args):
    ...
    The STN is loaded once and shared across all seeds.
    No metrics are computed — outputs are for downstream uncertainty analysis only.
    """
    print(separator)
    print('Starting UNCERTAINTY ESTIMATION...')
    print(separator)

    # Load crop_size saved by train() so every seed uses identical STN input dimensions.
    crop_size_path = os.path.join(args.model, 'crop_size.json')
    if os.path.exists(crop_size_path):
        with open(crop_size_path) as f:
            saved_crop_size = json.load(f)['crop_size']
        print(f'[UNCERTAINTY] Loaded crop_size={saved_crop_size} from {crop_size_path}')
    else:
        saved_crop_size = None
        print(f'[UNCERTAINTY] crop_size.json not found — falling back to config size')

    config = set_up_model_and_preprocessing('UNCERTAINTY', args, size_override=saved_crop_size)
    print(f'[UNCERTAINTY] STN input_size={config.config["size"]}, Anatomix roi_size={config.config["anatomix_roi_size"]}')

    # Load the trained STN once; it stays frozen for all seeds.
    config.stn.load_state_dict(torch.load(args.model + '/stn.pt'))
    config.stn.eval()

    # Pre-load atlas tensors — identical for every seed and every image.
    atlas_img_sitk = sitk.ReadImage(args.model + '/atlas_image_final.nii.gz')
    atlas_lab_sitk = sitk.ReadImage(args.model + '/atlas_labelmap_final.nii.gz')

    atlas_img_t = torch.from_numpy(
        sitk.GetArrayFromImage(atlas_img_sitk)
    )[None, None, ...].to(config.device)  # (1, 1, D, H, W)

    if len(atlas_img_t.size()) == 5:
        atlas_lab_t = torch.from_numpy(
            sitk.GetArrayFromImage(atlas_lab_sitk)
        ).permute(3, 0, 1, 2).unsqueeze(0).to(config.device)  # (1, C, D, H, W)
    else:
        atlas_lab_t = torch.from_numpy(
            sitk.GetArrayFromImage(atlas_lab_sitk)
        ).permute(2, 0, 1).unsqueeze(0).to(config.device)  # (1, C, H, W)

    out_root = os.path.join(args.out, 'uncertainty')
    ...
    for ckpt_path in args.anatomix_checkpoints:
        ...
        seed_out_dir = os.path.join(out_root, seed_label)
        os.makedirs(seed_out_dir, exist_ok=True)

        # Build dataset for this seed's Anatomix checkpoint.
        # fixed_crop_size=saved_crop_size ensures the crop SIZE matches the trained STN.
        # The crop centre (centroid) will differ per seed — that is intentional.
        dataset_seed = ImageSegmentationOneHotDataset(
            args.test,
            config.config['num_classes'],
            config.config['anatomix_roi_size'],
        ...
        with torch.no_grad():
            for index, batch_samples in enumerate(tqdm(dataloader_seed,
                                                        desc=f'Seed {seed_label}')):
                image_prime = batch_samples['labelmap'].to(config.device)  # (1, C, D, H, W)
                ...
                warped_atlas_lab = config.stn.warp_inv_image(atlas_lab_t)  # (1, C, D, H, W)
                transform     = config.stn.get_T()
                transform_inv = config.stn.get_T_inv()

                ref_sitk = dataset_seed.get_sample(index)['labelmap']

                # warped atlas labelmap — full one-hot (for soft uncertainty maps)
                if args.mode3d:
                    wal_sitk = sitk.GetImageFromArray(
                        warped_atlas_lab.cpu().squeeze().detach().permute(1, 2, 3, 0).numpy(),
                        isVector=True)
                    ip_sitk = sitk.GetImageFromArray(
                        image_prime.cpu().squeeze().detach().permute(1, 2, 3, 0).numpy(),
                        isVector=True)
                else:
                    wal_sitk = sitk.GetImageFromArray(
                        warped_atlas_lab.cpu().squeeze().detach().permute(1, 2, 0).numpy(),
                        isVector=True)
                    ip_sitk = sitk.GetImageFromArray(
                        image_prime.cpu().squeeze().detach().permute(1, 2, 0).numpy(),
                        isVector=True)

                # argmax versions — directly usable for voxel-wise uncertainty
                wal_argmax_sitk = sitk.GetImageFromArray(
                    torch.argmax(warped_atlas_lab, dim=1)
                        .cpu().squeeze().detach().numpy().astype(np.float32))
                ip_argmax_sitk = sitk.GetImageFromArray(
                    torch.argmax(image_prime, dim=1)
                        .cpu().squeeze().detach().numpy().astype(np.float32))

                # deformation fields
                T_sitk = sitk.GetImageFromArray(
                    transform.cpu().squeeze().detach().numpy(), isVector=True)
                T_inv_sitk = sitk.GetImageFromArray(
                    transform_inv.cpu().squeeze().detach().numpy(), isVector=True)
                ...
                for sitk_img, suffix in [
                    ...
                    (ip_sitk,         '_image_prime.nii.gz'),
                    (ip_argmax_sitk,  '_image_prime_argmax.nii.gz'),
                    (T_sitk,          '_transform.nii.gz'),
                    (T_inv_sitk,      '_transform_inv.nii.gz'),
                ]:
                    sitk_img.CopyInformation(ref_sitk)
                    sitk.WriteImage(sitk_img, prefix + suffix)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='coronary atlas-ISTN (per-fold, leakage-safe)')
    ...
    # Fold identity — drives all default paths when explicit paths are omitted.
    parser.add_argument('--fold', default='fold_1',
                        help='cross-validation fold identifier, e.g. fold_1 ... fold_5 (default: fold_1)')

    # Anatomix checkpoint — explicitly supply the fold-k checkpoint trained on fold-k train patients.
    # If omitted, auto-discovers the lowest-loss finetuned checkpoint under
    # saved_models/segmentation_asoca/<fold>/.
    parser.add_argument('--anatomix_checkpoint', default=None,
                        help='Anatomix checkpoint for this fold (auto-discovered if not set)')

    # ASOCA / Coronary
    # Data args — all default to None and are derived from --fold after parsing.
    parser.add_argument('--train', default=None, help='training data csv file')
    parser.add_argument('--val', default=None, help='validation data csv file (default: None)')
    ...
    parser.add_argument('--model', default=None, help='model directory')
    ...
    parser.add_argument('--anatomix_checkpoints', nargs='+', default=None,
                        help='Per-seed Anatomix checkpoint paths for multi-seed uncertainty estimation')
    ...
    # Derive all fold-specific defaults after parsing.
    base_cfg = 'data/config/asoca'
    base_out = f'output/asoca-coronary/{args.fold}/full-stn'

    if args.config is None:
        args.config = f'{base_cfg}/config.json'
    if args.train is None:
        args.train = f'{base_cfg}/{args.fold}/train.csv'
    if args.val is None:
        args.val = f'{base_cfg}/{args.fold}/test.csv'
    if args.test is None:
        args.test = f'{base_cfg}/{args.fold}/test.csv'
    if args.out is None:
        args.out = base_out
    if args.model is None:
        args.model = f'{base_out}/train/model'

    # Resolve single Anatomix checkpoint (used by train + test).
    # test_uncertainty uses args.anatomix_checkpoints (plural) instead.
    if args.anatomix_checkpoint is None and args.anatomix_checkpoints is None:
        args.anatomix_checkpoint = _find_best_asoca_fold_checkpoint(args.fold)

    print(separator)
    print(f'Coronary Atlas-ISTN  |  fold={args.fold}')
    print(f'  config:              {args.config}')
    print(f'  train CSV:           {args.train}')
    print(f'  val CSV:             {args.val}')
    print(f'  test CSV:            {args.test}')
    print(f'  Anatomix checkpoint: {args.anatomix_checkpoint}')
    print(f'  output root:         {args.out}')
    print(f'  model dir:           {args.model}')
    print(separator)

    # Run training
    if args.train is not None and args.anatomix_checkpoints is None:
        train(args)

    # Run testing
    if args.test is not None and args.anatomix_checkpoints is None:
        test(args)

    # Run multi-seed uncertainty estimation (skips train/test entirely)
    if args.anatomix_checkpoints is not None:
