...
# # Extract ensemble-averaged radiomic and deformation features
#
# Ensemble mean only experiment:
#   For each subject, extract geometric + radiomics features under each of 5 Anatomix
#   segmentation seeds, then take the per-feature mean across seeds.
...
for key in sorted(_feature_bank.keys()):
    ...
    # Average geometric singular values (3 per structure)
    struct_evr_mean = {}
    for L in class_mapping.keys():
        struct_evr_mean[L] = np.mean(entry["geo"][L], axis=0)  # (MAX_DEF_PC,)

    # Average radiomics; NaN-fill seeds where the mask was empty
    radiomics_mean = {}
    for L in class_mapping.keys():
        ...
        if SEMANTIC_FEATURES:
            ...
            radiomics_mean[L] = np.nanmean(nan_filled, axis=0)
        else:
    ...
    subjects.append({
        ...
        "label":      entry["label"],
        "split":      entry["split"],
        "struct_evr": struct_evr_mean,  # {L: (MAX_DEF_PC,)} — ensemble-averaged SVD singular values
        "radiomics":  radiomics_mean,   # {L: (F,)} — ensemble-averaged radiomics
    })
...
# Each entry's features are ensemble-averaged over SEEDS seeds — NOT single-seed estimates.
...
for subj in subjects:
    ...
    # Geometric: ensemble-averaged SVD singular values — stored under def_pc{i+1}_{name}
    for L, name in class_mapping.items():
        evr = subj["struct_evr"][L]  # (MAX_DEF_PC,) — ensemble mean of per-seed singular values
        for pc_idx in range(MAX_DEF_PC):
            col = f"def_pc{pc_idx+1}_{name}"
            row[col] = float(evr[pc_idx])

    # Radiomics: ensemble-averaged per structure
    for L, name in class_mapping.items():
        rad_vec = subj["radiomics"][L]  # (len(SEMANTIC_FEATURES),) — ensemble mean
...
print(f"\n=== Feature dimensionality (ensemble_mean_only) ===")
print(f"  Radiomic features : {len(SEMANTIC_FEATURES)} per structure × {_n_structs} = {_n_rad_feats}  (ensemble-averaged)")
print(f"  Geometric features: {MAX_DEF_PC} PCs × {_n_structs} = {_n_geo_feats}  (ensemble-averaged SVD singular values)")
...
# ensemble_mean_only: no uncertainty scalars — geo+radiomics ensemble means only.
