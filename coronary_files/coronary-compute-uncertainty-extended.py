...
Soft labelmap NIfTI files have shape (D, H, W, 1, C) where the axis-3 singleton is
a saved batch dimension. All internal computation uses (D, H, W, C).

Per-structure uncertainty uses per-class binary entropy with soft-mask thresholding
to avoid dilution from near-zero background voxels.  A threshold sweep over multiple
probability thresholds selects the one that best discriminates per-structure entropy.
"""
...
def compute_binary_class_entropy(prob_map: np.ndarray, eps: float = 1e-8) -> np.ndarray:
    ...
    Treats each voxel as an independent Bernoulli variable with success
    probability p (predicted probability for this class):

        H_c = -p * log(p) - (1-p) * log(1-p)

    Unlike multiclass entropy averaged over argmax voxels, this captures
    uncertainty at class boundaries and near-miss voxels with non-trivial
    but sub-0.5 probability — voxels the argmax mask would exclude entirely.
...
def compute_threshold_sweep(
    ...
    for class_id in range(1, C):  # skip background
        ...
        h = compute_binary_class_entropy(p)  # (D, H, W)
...
def print_threshold_summary(
    ...
    """Print a concise per-threshold summary table aggregated over all samples."""
    print("\n--- Threshold Sweep Summary (aggregated over all samples) ---")
    header = f"  {'τ':>7}  {'Mean H':>10}  {'Std H':>9}  {'CV':>8}  {'Min voxels':>11}"
    print(header)
    print("  " + "-" * (len(header) - 2))
    ...
    for tau in thresholds:
        ...
        if entropies:
            ...
            mean_e = arr.mean()
            std_e = arr.std()
            cv = std_e / mean_e if mean_e > 0 else float("nan")
        else:
            mean_e = std_e = cv = float("nan")

        min_v = min(voxel_counts) if voxel_counts else 0
        marker = "  <-- recommended" if tau == selected_tau else ""
        print(f"  {tau:>7.3f}  {mean_e:>10.4f}  {std_e:>9.4f}  {cv:>8.4f}  {min_v:>11d}{marker}")

    print(f"\nRecommended threshold: τ = {selected_tau}")
    print(
        "Rationale: maximises coefficient of variation of per-structure binary entropy\n"
...
def process_sample(
    ...
    # mean_probs shape (D, H, W, C) — batch singleton already removed in loader
    save_nifti(
        mean_probs,
        ref_img,
...
def validate_seeds(uncertainty_root: Path, seeds: list, sample_ids: list) -> None:
    """Confirm all required files exist across all seeds."""
    missing = []
    for seed in seeds:
...
def main():
    ...
    parser.add_argument(
        "--test_dir",
        default="",
        help="Directory containing sample_X_image.nii.gz CT volumes (optional; overlays skipped if absent).",
    )
    parser.add_argument(
        "--out_dir",
    ...
    # Full threshold sweep — all thresholds for downstream sensitivity analysis
    write_csv(
        all_sweep_rows,
        ["sample_id", "class_id", "class_name", "threshold", "mean_entropy", "voxel_count"],
        sweep_csv,
    ...
    # Per-case global multiclass entropy (backward-compatible)
    write_csv(
        all_per_case,
