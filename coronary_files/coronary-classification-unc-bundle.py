...
# # Coronary Geo-Radio Classification — Uncertainty Bundle  (Stage 3 — per-fold)
#
# Experiment: Baseline (3 geometric + 107 radiomics) PLUS 3 uncertainty bundle features.
#
#   def_variance               — mean magnitude of per-voxel variance in the STN
#                                transform field across 5 seeds (registration instability)
#   volume_variance            — variance of coronary mask voxel count across seeds
#                                (topology instability)
#   pairwise_dice_disagreement — mean pairwise (1 – Dice) across 10 seed pairs
#                                (segmentation inconsistency)
#
# Total features: 110 baseline + 3 uncertainty bundle = ~113 (Optuna searches def_pc_amt).
#
# Prerequisites (must run in order):
#   Stage 1 — coronary-seg-finetune.py
#   Stage 2 — coronary-atlas-istn.py   (fold-specific STN + atlas)
#   Unc inf — run_coronary_uncertainty_seeds.sh    (5-seed transform fields + argmax masks)
#   Bundle  — run_coronary_compute_unc_bundle.sh   (uncertainty_bundle_features.csv)
#
# Outputs (all written to classification_unc_bundle/ — does NOT overwrite other experiments):
#   features_unc_bundle.csv, predictions_unc_bundle.csv, metrics_unc_bundle.json,
#   results_unc_bundle.csv, mlp_unc_bundle.pt
#
# Usage:
#   python coronary-classification-unc-bundle.py --fold fold_1
...
# Uncertainty bundle experiment — always on in this script.
# All outputs go to classification_unc_bundle/ (never overwrites other experiments).
USE_UNC_BUNDLE = True
...
_bundle_train_csv = f"{_base_out}/full-stn-train-unc/uncertainty_analysis/metrics/uncertainty_bundle_features.csv"

_bundle_test_df   = pd.read_csv(_bundle_test_csv)
_bundle_train_df  = pd.read_csv(_bundle_train_csv)

print("\n=== Uncertainty Bundle CSVs — verification ===")
print(f"  Test  CSV : {_bundle_test_csv}  shape={_bundle_test_df.shape}")
print(f"  Train CSV : {_bundle_train_csv}  shape={_bundle_train_df.shape}")
...
_bundle_train_dict = {
    int(row["sample_id"]): {c: row[c] for c in _UNC_BUNDLE_COLS}
    for _, row in _bundle_train_df.iterrows()
}

_missing_test  = [s["sample_idx"] for s in subjects
                  if s["split"] == "test"  and s["sample_idx"] not in _bundle_test_dict]
_missing_train = [s["sample_idx"] for s in subjects
                  if s["split"] == "train" and s["sample_idx"] not in _bundle_train_dict]
if _missing_test or _missing_train:
    ...
    print(f"\n  All uncertainty bundle entries present. ✓")
...
for subj in subjects:
    ...
    # Uncertainty bundle features — split-conditional lookup.
    # train subjects use _bundle_train_dict; test subjects use _bundle_test_dict.
    sid    = subj["sample_idx"]
    _bdict = _bundle_test_dict if subj["split"] == "test" else _bundle_train_dict
    if sid in _bdict:
        for _uc in _UNC_BUNDLE_COLS:
            row[_uc] = float(_bdict[sid][_uc])
    else:
        for _uc in _UNC_BUNDLE_COLS:
            row[_uc] = float("nan")
...
print(f"\n=== Uncertainty bundle column ranges (sanity check — must NOT be near-constant) ===")
print(df_full[_UNC_BUNDLE_COLS].describe())

_n_structs   = len(class_mapping)
_n_rad_feats = len(SEMANTIC_FEATURES) * _n_structs
_n_geo_feats = MAX_DEF_PC * _n_structs
_n_unc_feats = len(_UNC_BUNDLE_COLS)  # def_variance, volume_variance, pairwise_dice_disagreement
...
print(f"  Uncertainty bundle    : {_n_unc_feats}  ({', '.join(_UNC_BUNDLE_COLS)})")
...
print(f"  ── USE_UNC_BUNDLE = {USE_UNC_BUNDLE}")
...
def objective(trial):
    ...
    # Append the 3 uncertainty bundle features.
    if USE_UNC_BUNDLE:
        selected_cols.extend(_UNC_BUNDLE_COLS)

    if trial.number == 0:
        bundle_in_sel = [c for c in selected_cols if c in _UNC_BUNDLE_COLS]
        print(f"\n[Trial 0] USE_UNC_BUNDLE={USE_UNC_BUNDLE}")
        print(f"[Trial 0] selected_cols count  : {len(selected_cols)}")
        print(f"[Trial 0] bundle cols present  : {bundle_in_sel}")

    X_df = df_full[selected_cols]
    X_np = X_df.to_numpy(dtype=np.float32)

    if trial.number == 0:
        print(f"[Trial 0] X_np.shape          : {X_np.shape}  ← bundle features "
              f"{'included' if USE_UNC_BUNDLE else 'excluded'}")
    y_np = y_global.copy()
...
if USE_UNC_BUNDLE:
    _sel_out.extend(_UNC_BUNDLE_COLS)
...
# predictions_unc_bundle.csv: test patients only
_pred_rows = []
...
_metrics_out = {
    ...
    "feature_count": {
        ...
        "uncertainty_bundle": _n_unc_feats,
...
#   (a) First-layer weight magnitude for each bundle feature vs all others.
#   (b) Permutation importance — accuracy drop when each bundle feature is permuted.

def analyse_bundle_importance():
    hp          = best_trial.user_attrs["hyperparams"]
    bundle_cols = _UNC_BUNDLE_COLS
    ...
    sel.extend(bundle_cols)

    X    = df_full[sel].to_numpy(dtype=np.float32)
    y    = y_global.copy()
    n_in = X.shape[1]
    bundle_indices = {col: sel.index(col) for col in bundle_cols}
    layers_fi = [hp["hidden_units"]] * hp["num_layers"] + [1]

    w_imp   = np.zeros(n_in)
    p_drops = {col: [] for col in bundle_cols}
    n_folds = 0
    ...
    for col, idx in bundle_indices.items():
        print(f"  {col} index in X: {idx}")
    ...
    for train_i, val_i in skf_fi.split(np.zeros((len(y), 1)), y):
        ...
        for col, col_idx in bundle_indices.items():
            Xvl_perm = Xvl_s.copy()
            Xvl_perm[:, col_idx] = rng.permutation(Xvl_perm[:, col_idx])
            with torch.no_grad():
    ...
    for col, col_idx in bundle_indices.items():
        print(f"  {col:<40}  weight mag: {float(w_imp[col_idx]):.6f}")
    ...
    for rank, (fi, mag) in enumerate(ranked[:10], 1):
        tag = f"  ← BUNDLE ({sel[fi]})" if sel[fi] in bundle_cols else ""
